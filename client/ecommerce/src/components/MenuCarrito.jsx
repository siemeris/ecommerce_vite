    ...
    <>
      ...
      <div className="right-0 bottom-0 w-4/5 h-screen z-30 bg-blue-100 pt-7 fixed overflow-scroll">
        ...
        <div className="w-full flex justify-between content-center border-b pb-5 border-gray-300">
          ...
          <svg onClick={() => setMenuCarritoVisible(!menuCarritoVisible)} className="w-5 h-5 m-auto hover:cursor-pointer  transition hover:text-red-600" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </div>
        {/* Contenedor de toda la lista de productos del carrito */}
        <div className="w-full overflow-scroll absolute z-50 bg-white">
          {/* Productos */}
          {itemsCarrito.map((producto) => (
            <MenuCarritoProducto key={producto.id} producto={producto} />
          ))}

          {/* Lista de detalle del carrito */}
          <div className="bg-gray-200 px-4 w-full bottom-0 flex flex-col">
            <div className="w-full mb-5">
              {itemsCarrito.map((producto) => {
                return (
                  <MenuCarritoDetalle key={producto.id} producto={producto} />
                );
              })}
