...
const MenuCarritoProducto = ({producto}) => {
    ...
    const {title, price, photos, cantidad} = producto

  return (
    <div className="flex border-b border-gray-300 py-2 items-center">
      <img className="object-contain w-1/3 max-h-24" src={photos[0]} alt=""/>
      <div className="w-2/3 p-5">
        <div>
          <h3 className="text-start text-gray-800 font-bold text-sm">{title}</h3>
          <p className="text-start text-sm text-blue-700">Cantidad: <span>{cantidad}</span></p>
          <p className="text-start text-sm text-gray-800">Negro</p>
          <div 
              onClick={() => eliminarProducto(producto)}
              className="w-20 flex justify-start content-center items-center gap-1 hover:cursor-pointer">
